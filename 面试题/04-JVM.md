...
2. **分析 GC 日志**：开启 `-XX:+PrintGCDetails -XX:+PrintGCDateStamps -Xloggc:gc.log`，看每次 Full GC 前后老年代的回收情况。如果回收后老年代占用依然很高，说明有大量不可回收对象（可能是内存泄漏）。
...
2. 搜索 `deadlock` 关键字
3. 如果 jstack 没有检测到（比如是 ReentrantLock 死锁，JDK 较老版本可能检测不到），手动搜索 BLOCKED 状态的线程，分析它们等待的锁和持有的锁
...
主要区别：
- JDK 9+ 用 `-Xlog` 统一了所有 JVM 日志，语法更规范
- 支持更灵活的日志级别控制（info、debug、trace）
- 支持按 tag 过滤（gc、gc+heap、gc+age 等）
- JDK 8 的 `-XX:+PrintGCDetails` 等参数在 JDK 9+ 中已废弃
...
**Concurrent Mode Failure** 发生在 CMS 并发清理阶段，老年代空间不足以容纳新晋升的对象。此时 CMS 会退化为 Serial Old 收集器进行 Full GC（单线程、STW），暂停时间会非常长。

触发原因：
- CMS 并发回收的速度跟不上对象分配的速度
- 老年代碎片化严重，虽然总空间够但没有连续空间分配大对象
- `-XX:CMSInitiatingOccupancyFraction` 设置过高，CMS 启动太晚

解决方案：
```bash
# 1. 降低 CMS 触发阈值，让 CMS 更早启动回收
-XX:CMSInitiatingOccupancyFraction=70    # 老年代使用 70% 时触发
-XX:+UseCMSInitiatingOccupancyOnly       # 只用设定的阈值，不让 JVM 自适应

# 2. 开启碎片整理（但会增加 STW 时间）
-XX:+UseCMSCompactAtFullCollection        # Full GC 时压缩
-XX:CMSFullGCsBeforeCompaction=2          # 每2次 Full GC 后压缩一次
...
**Full GC 触发条件**（G1 的 Full GC 是单线程的，JDK 10 后改为多线程）：
...
| **jcmd** | 万能工具，可替代上面大部分工具 | `jcmd <pid> help`（查看支持的命令） |
| **jhat** | 分析堆转储文件（已过时，建议用 MAT） | `jhat heap.hprof` |
...
`jcmd` 是 JDK 7 引入的万能诊断工具，Oracle 官方推荐用它替代 jmap、jstack 等。
...
1. **GC STW**：
   ...
   - 特别关注 Full GC 和 CMS 的 remark 阶段
